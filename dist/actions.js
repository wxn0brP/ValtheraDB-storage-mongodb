...
export class MongoDbAction extends ActionsBase {
    ...
    _applyFindOpts(data, findOpts) {
        ...
        if (Array.isArray(data))
            return data.map(doc => updateFindObject({
                ...doc,
            }, findOpts));
    ...
    async find(query) {
        ...
        const opts = this._session
            ...
            : {};
        const { reverse = false, offset = 0, limit = -1, sortBy, sortAsc = true, min, max, avg, groupBy, count, sum, distinct, } = dbFindOpts;
        const needsAggregation = min || max || avg || groupBy || count || sum || distinct;
        const searchIsFunc = typeof search === "function";
        const needsNativeFallback = !searchIsFunc && !needsJsFallback(search);
        const canUseNative = needsNativeFallback &&
            !needsAggregation &&
            sortBy !== "random()" &&
            !(reverse && !sortBy);
        if (canUseNative) {
            ...
            if (sortBy) {
                if (Array.isArray(sortBy)) {
                    ...
                    for (const s of sortBy) {
                        sortObj[s.field] = s.asc === false ? -1 : 1;
                    }
    ...
    async update(query) {
        ...
        if (typeof updater === "function") {
            const { filter, allData } = await resolveSearch(search, coll, context, opts);
    ...
    async removeOne(query) {
        ...
        const result = await coll.findOne(filter, opts);
        if (result)
            await coll.deleteOne(filter, opts);
        return cleanDocs(result);
    }
    ...
    async commitTransaction(handle) {
        const session = handle._adapterData;
